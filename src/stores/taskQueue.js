...
export const useTaskQueueStore = defineStore('taskQueue', {
    ...
    actions: {
         ...
         * @returns {string|null} 任务ID，如果重复则返回null
         */
        addTask(taskData) {
            // 检查是否已存在相同的任务（同一个historyId + 同一个type，且状态为排队中或执行中）
            // 注意：只有当historyId存在时才检查重复（本地任务没有historyId）
            if (taskData.historyId) {
                const existingTask = this.tasks.find(t =>
                    t.historyId === taskData.historyId &&
                    t.type === taskData.type &&
                    (t.status === TASK_STATUS.QUEUED || t.status === TASK_STATUS.RUNNING)
                )

                if (existingTask) {
                    const typeText = taskData.type === 'extract' ? '文案提取' :
                        taskData.type === 'rewrite' ? '文案改写' : '下载'
                    toast.warning(`该视频已有${typeText}任务正在进行中`)
                    return null
                }
            }
            ...
            try {
                ...
                if (task.data?.isLocal && task.data?.localType === 'audio') {
                    const {
                        ...
                        checkAudioExists
                    } = await import('@/services/storage/localAudioStorage.js')
