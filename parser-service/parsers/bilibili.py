...
class BilibiliParser:
    ...
    def __init__(self, url: str, cookie: str = None):
        ...
        print(f"[Bilibili] Cookie 包含 SESSDATA: {'SESSDATA' in self.cookie if self.cookie else False}")
        print(f"[Bilibili] Cookie 包含 buvid3: {'buvid3' in self.cookie if self.cookie else False}")
    ...
    def _get_wbi_keys(self) -> tuple:
        ...
        try:
            ...
            if img_key and sub_key and len(img_key) >= 32 and len(sub_key) >= 32:
                ...
                print(f"[Bilibili] WBI keys obtained: img_key={img_key[:8]}..., sub_key={sub_key[:8]}...")
                return self.wbi_keys
            else:
                print(f"[Bilibili] WBI keys 无效: img_key长度={len(img_key)}, sub_key长度={len(sub_key)}")
        except Exception as e:
    ...
    def _wbi_sign(self, params: dict) -> str:
        ...
        for k, v in sorted_params:
            ...
            query_parts.append(f"{urllib.parse.quote(str(k), safe='')}={urllib.parse.quote(v_str, safe='')}")
    ...
    def _get_play_url(self) -> Optional[dict]:
        ...
        try:
            ...
            print(f"[Bilibili] 登录状态: {'已登录' if self.is_login else '未登录'}, qn={params['qn']}, fnval={params['fnval']}")
            ...
            if data.get('code') == 0:
                ...
                print(f"[Bilibili] accept_quality: {play_data.get('accept_quality', [])}")
                print(f"[Bilibili] accept_description: {play_data.get('accept_description', [])}")
                
                for i, v in enumerate(videos):
                    print(f"[Bilibili] 视频流[{i}]: id={v.get('id')}, {v.get('width')}x{v.get('height')}, codecs={v.get('codecs', '')[:20]}")
    ...
    def get_video_info(self) -> Optional[dict]:
        ...
        try:
            ...
            if play_data:
                ...
                if videos:
                    ...
                    for v in videos:
                        ...
                        backup_urls = v.get('backupUrl', []) or v.get('backup_url', []) or []
                        width = v.get('width', 0)
                        ...
                        short, name = quality_map.get(quality_id, (f'{height}P', f'{height}P'))
                        estimated_size = (bandwidth / 8) * duration if bandwidth and duration else 0
                ...
                dolby_audio = dolby_data.get('audio', []) or [] if isinstance(dolby_data, dict) else []
                flac_data = dash.get('flac') or {}
                flac_audio = flac_data.get('audio') if isinstance(flac_data, dict) else None
                ...
                if all_audios:
                    best_audio = max(all_audios, key=lambda x: x.get('bandwidth', 0) or x.get('id', 0))
            ...
            # 格式化发布时间
            create_time = datetime.fromtimestamp(pubdate).strftime('%Y-%m-%d %H:%M:%S') if pubdate else ''
            
            # 支持的清晰度列表
            accept_quality = play_data.get('accept_quality', []) if play_data else []
            accept_description = play_data.get('accept_description', []) if play_data else []
            
            print(f"[Bilibili] 解析完成: 视频流{len(video_streams)}个, 音频流{'1个' if audio_stream else '0个'}")
